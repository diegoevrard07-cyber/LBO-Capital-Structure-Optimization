...
FRONTIER_MC_DRAWS = (
    1000  # per leverage level; keeps the frontier interactive (~1pp precision on distress)
)
...
def terminal_table(
    headers: list[str], rows: list[list[tuple[str, str]]], compact: bool = False
) -> str:
...
def heatmap_tearsheet(opt: OptimizationResult) -> go.Figure:
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            showscale=True,
            colorbar=dict(
                title=dict(text="IRR %", font=dict(size=9)),
    ...
    terminal_layout(
        fig,
        430,
        "IRR sensitivity — total leverage × senior mix (base case, %)",
    )
    fig.update_layout(plot_bgcolor="#161c26")  # infeasible (NaN) cells read as dead panel
    fig.update_layout(
        margin=dict(l=48, r=44, t=34, b=34)
    )  # extra room so the colorbar never clips cell labels
...
def cap_stack_chart(stack, equity0: float, ccy: str) -> go.Figure:
    ...
    for t in stack:
        ...
        fig.add_trace(
            go.Bar(
                y=["CAP STACK"],
                x=[t.amount],
                name=f"{name} {t.turns:.2f}×",
                orientation="h",
                marker=dict(color=colors.get(name, STEEL)),
                text=f"{name.upper()} {t.turns:.2f}×",
                textposition="auto",
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", yanchor="top", y=-0.12, x=0, font=dict(size=8), title=None),
    )
...
def paydown_chart(yearly: pd.DataFrame, stack) -> go.Figure:
    ...
    for t in stack:
        ...
        fig.add_trace(
            go.Scatter(
                x=years,
                y=[t.amount] + yearly[f"balance_{name}"].tolist(),
                name=name,
                stackgroup="debt",
                mode="lines",
                line=dict(color=colors.get(name, STEEL), width=1.5),
    ...
    fig.add_trace(
        go.Scatter(
            x=years,
            y=[0.0] + yearly["cash_closing"].tolist(),
            name="Cash",
            mode="lines",
    ...
    fig = go.Figure(
        go.Waterfall(
            ...
            y=[
                ...
                attribution["multiple_expansion"],
                attribution["debt_paydown"],
                attribution["fees"],
                attribution["total_profit"],
            ],
...
def frontier_chart(
    f: pd.DataFrame, opt_mc: MCResult, naive_mc: MCResult | None, survivable_x: float
) -> go.Figure:
    ...
    fig.add_vrect(
        ...
        opacity=0.07,
        line_width=0,
        annotation_text="SURVIVABLE ZONE",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate=(
                "Leverage %{customdata[0]:.1f}×<br>Median IRR %{y:.1%}<br>"
...
def covenant_chart(yearly: pd.DataFrame) -> go.Figure:
    fig = make_subplots(
        cols=2, subplot_titles=("INTEREST COVERAGE VS MINIMUM", "NET LEVERAGE VS COVENANT")
    )
    fig.add_trace(
        go.Scatter(
            x=yearly["year"],
            y=yearly["coverage_ratio"],
            name="Coverage",
            mode="lines+markers",
    ...
    fig.add_trace(
        go.Scatter(
            x=yearly["year"],
            y=yearly["coverage_covenant"],
            name="Minimum",
            mode="lines",
            line=dict(color=NEG, dash="dash", width=1),
        ),
        row=1,
        col=1,
    )
...
def mc_histogram(mc: MCResult) -> go.Figure:
    fig = go.Figure(
        go.Histogram(
            x=mc.irrs, nbinsx=60, marker=dict(color=STEEL, line=dict(color=BORDER, width=0.5))
        )
    )
    for value, label, color in [
        (mc.p5, "P5", NEG),
        (mc.median, "MEDIAN", ACCENT),
        (mc.p95, "P95", POS),
    ]:
    ...
    terminal_layout(
        fig, 380, f"IRR distribution — {mc.n:,} draws (surviving; wiped equity at −100%)"
    )
    fig.update_layout(
        xaxis_tickformat=".0%",
        xaxis_title=dict(text="IRR", font=dict(size=9)),
...
def su_table_html(entry: dict, stack, ccy: str) -> str:
    uses = [
        ("PURCHASE PRICE (EV)", entry["ev"]),
        ("FEES", entry["fees"]),
        ("TOTAL USES", entry["ev"] + entry["fees"]),
    ]
    sources = [(f"{t.tranche.name.upper()} {t.turns:.2f}×", t.amount) for t in stack]
    sources += [
        ("SPONSOR EQUITY", entry["equity"]),
        ("TOTAL SOURCES", entry["total_debt"] + entry["equity"]),
    ]
    rows = []
    for i in range(max(len(uses), len(sources))):
        ...
        rows.append(
            [
                (u[0], "muted" if not u_cls else u_cls),
                (fmm(u[1], ccy) if u[1] is not None else "", u_cls),
                (s[0], "muted" if not s_cls else s_cls),
                (fmm(s[1], ccy) if s[1] is not None else "", s_cls),
            ]
        )
...
def sidebar_assumptions() -> MarketAssumptions:
    ...
    with st.sidebar.expander("Rates & tranches", expanded=False):
        base_rate = st.number_input(
            "Base rate (SONIA proxy)", 0.0, 0.15, d.base_rate, 0.0025, format="%.4f"
        )
        senior_margin = st.number_input(
            "Senior TLB margin", 0.0, 0.10, d.senior_margin, 0.0025, format="%.4f"
        )
        second_lien_margin = st.number_input(
            "Second-lien margin", 0.0, 0.15, d.second_lien_margin, 0.0025, format="%.4f"
        )
        mezz_rate = st.number_input(
            "Mezzanine fixed coupon", 0.0, 0.25, d.mezz_rate, 0.005, format="%.3f"
        )
        senior_cap = st.number_input("Senior cap (turns)", 0.0, 8.0, d.senior_cap, 0.25)
        second_lien_cap = st.number_input(
    ...
    with st.sidebar.expander("Covenants", expanded=False):
        ...
        headroom = st.number_input(
            "Leverage covenant headroom (turns)", 0.0, 2.0, d.leverage_covenant_headroom, 0.25
        )
        stepdown = st.number_input(
            "Leverage step-down per year", 0.0, 1.0, d.leverage_stepdown, 0.25
        )
    ...
    with st.sidebar.expander("Deal structure", expanded=False):
        ...
        entry_premium = st.number_input(
            "Entry premium over market (turns)", 0.0, 4.0, d.entry_premium_turns, 0.25
        )
        exit_premium = st.number_input(
            "Exit multiple vs entry (turns)", -3.0, 3.0, d.exit_multiple_premium, 0.25
        )
        sweep = (
            st.slider("Cash sweep (% of post-amort FCF)", 0, 100, int(d.cash_sweep_pct * 100)) / 100
        )
        debt_fee = st.number_input(
            "Debt fees (% of debt)", 0.0, 0.06, d.debt_fee_pct, 0.005, format="%.3f"
        )
        ev_fee = st.number_input(
            "Transaction fees (% of EV)", 0.0, 0.05, d.ev_fee_pct, 0.005, format="%.3f"
        )
        wc = st.number_input(
            "Working capital (% of incremental revenue)",
            0.0,
            0.10,
            d.wc_pct_of_delta_rev,
            0.01,
            format="%.2f",
        )
    ...
    with st.sidebar.expander("Stress & Monte Carlo", expanded=False):
        g_haircut = (
            st.slider("Stress growth haircut (%)", 0, 60, int(d.stress_growth_haircut * 100)) / 100
        )
        e_shock = (
            st.slider("Stress year-1 EBITDA shock (%)", 0, 30, int(d.stress_ebitda_shock * 100))
            / 100
        )
        x_shock = st.number_input(
            "Stress exit-multiple shock (turns)", 0.0, 4.0, d.stress_exit_multiple_shock, 0.25
        )
        draws = st.number_input("Monte Carlo draws", 1000, 20000, d.mc_draws, 1000)

    return MarketAssumptions(
        base_rate=base_rate,
        senior_margin=senior_margin,
        second_lien_margin=second_lien_margin,
        mezz_rate=mezz_rate,
        ...
        hold_years=int(hold),
        entry_premium_turns=entry_premium,
        exit_multiple_premium=exit_premium,
        cash_sweep_pct=sweep,
        debt_fee_pct=debt_fee,
        ev_fee_pct=ev_fee,
        wc_pct_of_delta_rev=wc,
        stress_growth_haircut=g_haircut,
        stress_ebitda_shock=e_shock,
        stress_exit_multiple_shock=x_shock,
        mc_draws=int(draws),
    )
...
def main() -> None:
    ...
    metric_strip(
        [
            ...
            (
                "ENTRY EBITDA",
                fmm(company.ebitda, company.currency),
                "",
                f"REV {fmm(company.revenue, company.currency)}",
            ),
            (
                "MKT EV/EBITDA",
                fmult(company.ev_ebitda, 1),
                "",
                f"+{assumptions.entry_premium_turns:.1f}× premium",
            ),
            ...
            with st.status("Optimizing capital structure…", expanded=True) as status:
                ...
                if opt.optimum is not None:
                    ...
                    if opt.naive is not None and opt.naive.stack is not None:
                        naive_mc = monte_carlo(
                            company, assumptions, opt.naive.stack, n=FRONTIER_MC_DRAWS
                        )
    ...
    if opt.optimum is None:
        st.error(
            "NO ADMISSIBLE CAPITAL STRUCTURE — no leverage × mix combination is feasible in the "
    ...
    metric_strip(
        [
            (
                ...
                fpct(base.irr),
                "accent",
                f"P5/P95 {fpct(res['opt_mc'].p5)}/{fpct(res['opt_mc'].p95)}",
            ),
            (
                "MOIC",
                fmult(base.moic),
                "accent",
                f"profit {fmm(base.attribution['total_profit'], company.currency)}",
            ),
            ("LEVERAGE @ CLOSE", fmult(o.leverage), "accent", f"{o.mix:.0%} senior mix"),
            (
                "ENTRY → EXIT",
                f"{fmult(base.entry['entry_multiple'], 1)} → {fmult(base.exit['exit_multiple'], 1)}",
                "",
            ...
            (
                "MIN COVERAGE",
                fmult(min_cov),
                "pos" if min_cov >= assumptions.min_interest_coverage else "neg",
                f"covenant {assumptions.min_interest_coverage:.1f}×",
            ),
            (
                "EQUITY CHECK",
                fmm(base.entry["equity"], company.currency),
                "",
                f"of {fmm(base.entry['ev'] + base.entry['fees'], company.currency)} uses",
            ),
        ]
    )
    ...
    with side_col:
        with st.container(border=True):
            ...
            st.markdown(
                su_table_html(base.entry, o.stack, company.currency), unsafe_allow_html=True
            )
    ...
    with st.container(border=True):
        st.markdown(
            '<div class="panel-title">Credit statistics by year — base case</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
    ...
    with mc_right:
        ...
        comp_rows = [
            [
                ("IRR", "muted"),
                (fpct(base.irr), "accent"),
                (fpct(stress.irr) if stress.feasible and stress.irr is not None else "n/a", ""),
            ],
            [
                ("MOIC", "muted"),
                (fmult(base.moic), "accent"),
                (
                    (
                        fmult(stress.moic)
                        if stress.feasible and stress.moic is not None
            ...
            [
                ("MIN COVERAGE", "muted"),
                (fmult(base.yearly["coverage_ratio"].min()), ""),
                (
                    (
                        fmult(stress.yearly["coverage_ratio"].min())
                        if not stress.yearly.empty
                        else "n/a"
                    ),
                    "",
                ),
            ...
            [
                ("MAX NET LEVERAGE", "muted"),
                (fmult(base.yearly["leverage_ratio"].max()), ""),
                (
                    (
                        fmult(stress.yearly["leverage_ratio"].max())
                        if not stress.yearly.empty
                        else "n/a"
                    ),
                    "",
        ...
        with st.container(border=True):
            st.markdown(
                '<div class="panel-title">Base vs stressed downside</div>', unsafe_allow_html=True
            )
